    ...
    <>
      {props.list.map((item) => (
        <div
          ...
          key={item._id}
        >
          <div className={styles['post-list-item__mark']}>{Mapping.postIcon[item.post_type]}</div>
          {item.post_cover && (
            <div
              className={styles['post-list-item__banner']}
              style={{
                backgroundImage: `url(//${item.post_cover && item.post_cover.media_url_720p})`,
              }}
            />
          )}
          <div className={styles['post-list-item__content']}>
            <div className={styles['post-list-item__title']}>
              {item.post_type === 1 &&
                `${item.post_title} ${item.movie_name_en} (${moment(item.movie_time).format(
                  'YYYY',
                )})`}
            ...
            <div className={styles['post-list-item__info']}>
              <li className={styles['post-list-item__info-item']}>
                ...
                &nbsp;
                <Link
                  to={`/authors/${encodeURI(item.post_author.user_name)}`}
              ...
              <li className={styles['post-list-item__info-item']}>
                ...
                &nbsp;
                <Link
                  to={Helper.getFullCategoryPathById(item.post_category._id, menu)}
                  className="link-light"
                >
                  {item.post_category.category_title}
                </Link>
              </li>
